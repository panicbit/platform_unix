...
impl traits::FileAttr<Std> for FileAttr {
...
impl traits::Permissions for FilePermissions {
...
impl traits::FileType for FileType {
...
impl traits::DirEntry<Std> for DirEntry {
...
impl traits::OpenOptions for OpenOptions {
    fn new() -> OpenOptions {
        OpenOptions {
            // generic
            read: false,
...
impl traits::File<Std> for File {
    ...
    fn truncate(&self, size: u64) -> io::Result<()> {
        #[cfg(target_os = "android")]
        return ::android::ftruncate64(self.0.raw(), size);
...
impl File {
    ...
    fn fd(&self) -> &FileDesc { &self.0 }

    fn into_fd(self) -> FileDesc { self.0 }
}

impl traits::DirBuilder<Std> for DirBuilder {
    fn new() -> DirBuilder {
...
pub fn readlink(p: &Path) -> io::Result<PathBuf> {
    ...
    loop {
        ...
        if buf_read != buf.capacity() {
            ...
            return Ok(PathBuf::from(OsString::from_inner(os_str::Buf::from_inner(buf))));
        }
...
pub fn canonicalize(p: &Path) -> io::Result<PathBuf> {
    ...
    Ok(PathBuf::from(OsString::from_inner(os_str::Buf::from_inner(buf))))
}

pub fn copy(from: &Path, to: &Path) -> io::Result<u64> {
    use ap::fs::{File, set_permissions};
    if !from.is_file() {
